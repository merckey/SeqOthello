...
_k_-mer files, each of which contains the set of _k_-mers extracted from
reads of the corresponding RNA-Seq experiment.
Currently the _k_-mer file is generated by [Jellyfish](https://github.com/gmarcais/Jellyfish) from fasta/fastq files.
...
1. Convert _k_-mer files to __SeqOthello__ binary format.
    ...
    ``--cutoff=N`` allows you to further filter the _k_-mers by their counts. Only _k_-mers with at least N counts are kept for building __SeqOthello__.

    ``--k`` sets the _k_-mer length in the input file.

    The following example converted each of _k_-mer files into binary format. All 21-mers with N=1 are kept for SeqOthello construction.
